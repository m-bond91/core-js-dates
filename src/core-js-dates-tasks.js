...
function getNextFriday(date) {
  ...
  const diffDays = day < 5 ? 5 - day : 7 - day + 5;
  const nextFriday = new Date(date);
  nextFriday.setDate(date.getDate() + diffDays);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = Math.round((secondDate - firstDate) / msPerDay) + 1;
  return diff;
}
...
function isDateInPeriod(date, period) {
  ...
  return targetDate >= startPeriod && targetDate <= endPeriod;
}
...
function formatDate(date) {
  ...
  const dayy = day.getUTCDate();
  const year = day.getUTCFullYear();
  const hours = day.getUTCHours();
  const minutes = String(day.getUTCMinutes()).padStart(2, '0');
  const seconds = String(day.getUTCSeconds()).padStart(2, '0');

  const ampm = hours >= 12 ? 'PM' : 'AM';
  const formattedHours = hours % 12 || 12;

  return `${month}/${dayy}/${year}, ${formattedHours}:${minutes}:${seconds} ${ampm}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    const dayOdWeek = new Date(year, month - 1, day).getDay();
    if (dayOdWeek === 0 || dayOdWeek === 6) weekendDays += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = Math.floor(
    (tempDate - firstDayOfYear) / (1000 * 60 * 60 * 24)
  );
